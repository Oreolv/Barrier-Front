<template>
  <div class="header">如您目前有紧急事情需要办理，我们将会第一时间派遣网格员满足您的需求。</div>
  <div class="content">
    <div class="content-item">
      <div class="content-item__title">事件类型</div>
      <div class="content-item__text">
        <nut-radiogroup v-model="formValues.type" direction="horizontal">
          <nut-radio shape="button" v-for="(i, idx) in agencyType" :key="idx" :label="i">
            {{ i }}
          </nut-radio>
        </nut-radiogroup>
      </div>
...
const submitFormValues = async () => {
  ...
  setTimeout(() => {
    redirectTo({
      url: '/pages/serve/children/agency/index',
    });
  }, 1000);
};
</script>
